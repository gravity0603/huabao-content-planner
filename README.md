...
- 旅行标题新增“标题事件测试”：纯粹描写颜色、位置和构图仍不合格；标题应让具体对象发生可见突变、反常动作、身份/关系错位，或给出真实规则悬念、季节利益和旅行玩法。`海滩变红毯？`、`村里下“金雨”`、`限时镀金的沙漠`、`会消失的公路`只用于说明机制，不得批量套壳。
- 日本元素为全品类一票否决项：候选对象、文案、图片和核心素材来源任一命中即淘汰，不以改称、隐去地名或弱化来源规避。
...
详见 [`CHANGELOG.md`](CHANGELOG.md)。当前版本：v6.3.8；已加入日本元素一票否决和旅行标题“景物出事了”事件钩子闸门。
